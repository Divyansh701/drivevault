/**
 * VehicleDetailPage (Guest-accessible)
 *
 * Public vehicle detail page accessible to both guests and authenticated users.
 *
 * Guests can view:  all vehicle specs, images, description, price, mileage
 * Guests cannot:   purchase, add to wishlist, contact seller, book test drive
 *
 * Authenticated users get full interaction (purchase, wishlist, etc.)
 * Admin users get an additional "Edit in Admin Panel" shortcut.
 */
...
import { Button, GuestBanner } from '@/components/ui';
...
function GuestActionCTA() {
  return (
    <div className="glass p-5 border border-amber-500/20 bg-amber-500/5 rounded-xl">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-amber-300 mb-1">Sign in to take action</p>
          <p className="text-xs text-surface-400 mb-3 leading-relaxed">
            Create a free account or sign in to purchase this vehicle, save it to your wishlist, or book a test drive.
          </p>
          <div className="flex gap-2 flex-wrap">
            <Link
              to={`/login?returnUrl=${encodeURIComponent(window.location.pathname)}`}
              className="px-4 py-2 text-xs font-semibold text-slate-900 bg-amber-500 hover:bg-amber-400 rounded-lg transition-colors"
            >
              Sign In
            </Link>
            <Link
              to="/register"
              className="px-4 py-2 text-xs font-semibold text-amber-400 border border-amber-500/30 hover:border-amber-400/60 rounded-lg transition-colors"
            >
              Create Account
            </Link>
          </div>
...
const VehicleDetailPage: React.FC = () => {
  ...
  const { isAdmin, isGuest, requireAuth, showLoginPrompt } = useAuth();
  ...
  // ── Fetch vehicle (works for guests — no auth needed) ──────────────────────
  ...
  // ── Purchase handler (authenticated users only) ────────────────────────────
  ...
  }, [vehicle, purchasing, requireAuth, showLoginPrompt]);
    ...
    return (
      <div className="page-container flex flex-col items-center justify-center min-h-[60vh] text-center">
        <div className="glass p-10 max-w-md">
          ...
          <Button variant="primary" onClick={() => navigate('/customer')}>← Back to Showroom</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Guest Banner — shown at the very top for unauthenticated users */}
      <GuestBanner />

      <div className="page-container">
        {/* ── Breadcrumb ────────────────────────────────────────────────── */}
        <nav className="flex items-center gap-2 text-sm text-surface-400 mb-8" aria-label="Breadcrumb">
          <Link to="/customer" className="hover:text-white transition-colors flex items-center gap-1.5">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                ...
                {vehicle.vin   && <SpecItem label="VIN"    value={<span className="font-mono text-xs">{vehicle.vin}</span>} />}
            ...
            {/* ── CTA area: differs for guest vs authenticated ─────────── */}
            {isGuest ? (
              /* Guest: show sign-in prompt instead of action buttons */
              <GuestActionCTA />
            ) : (
              /* Authenticated: show full action buttons */
              <div className="space-y-3">
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 w-12 h-12 rounded-xl border transition-all flex items-center justify-center ${
                      wishlisted
                        ...
                        : 'border-white/10 bg-surface-800/50 text-surface-400 hover:border-red-500/30 hover:text-red-400'
                    }`}
                  >
                    <svg className="w-5 h-5" fill={wishlisted ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
